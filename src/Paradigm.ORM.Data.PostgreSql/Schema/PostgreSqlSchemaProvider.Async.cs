...
{
    public partial class PostgreSqlSchemaProvider
    {
        ...
        {
            return (await this.ColumnQuery.ExecuteAsync($"TABLE_CATALOG='{database}' AND TABLE_NAME='{tableName}'")).Cast<IColumn>().ToList();
        }
        ...
        {
            return (await this.ConstraintQuery.ExecuteAsync($"kcu.TABLE_CATALOG='{database}' AND kcu.TABLE_NAME='{tableName}'")).Cast<IConstraint>().ToList();
        }
        ...
        {
            return (await this.ParameterQuery.ExecuteAsync($"routines.SPECIFIC_CATALOG='{database}' AND ROUTINE_NAME='{routineName}'")).Cast<IParameter>().ToList();
        }
