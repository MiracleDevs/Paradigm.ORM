...
namespace Paradigm.ORM.Data.SqlServer.Schema
{
    public partial class SqlSchemaProvider
    {
        ...
        {
            return (await this.ColumnQuery.ExecuteAsync($"sc.[table_catalog]='{database}' AND sc.[table_name]='{tableName}'")).Cast<IColumn>().ToList();
        }
        ...
        {
            return (await this.ParameterQuery.ExecuteAsync($"[specific_catalog]='{database}' AND [specific_name]='{routineName}'")).Cast<IParameter>().ToList();
        }
